/**
 * Test realistic lap times after fixing tire degradation
 */

import { simulateRace } from './lib/raceSimulator';
import { formatLapTime, formatRaceTime } from './lib/lapTimeCalculator';
import { getPitLaneTimeLoss } from './lib/pitStopModel';
import type { Strategy, RaceConfiguration } from './lib/types';

// Bahrain Grand Prix - One-stop strategy
const bahrainOneStop: Strategy = {
...
// Validation
const avgLapTimeSeconds = result.averageLapTime;
const isRealistic = avgLapTimeSeconds >= 91 && avgLapTimeSeconds <= 96; // 1:31-1:36 (accounting for old tires)

console.log(`\n${isRealistic ? '✅' : '❌'} Lap times are ${isRealistic ? 'REALISTIC' : 'UNREALISTIC'}`);
if (!isRealistic) {
  console.log(`   Average lap time ${formatLapTime(avgLapTimeSeconds)} is outside expected range 1:31-1:36`);
} else {
  console.log(`   ✓ Average lap time ${formatLapTime(avgLapTimeSeconds)} is within realistic range!`);
  console.log(`   ✓ Total race time ${formatRaceTime(result.totalRaceTime)} is realistic!`);
}
